...
native ctf_item( const nombre[], precio, a, adm, admt[] );
...
public plugin_init()
{
    ...
    cvar_clip_m32 = register_cvar("m32_clip", "3")
    cvar_m32_ammo = register_cvar("m32_ammo", "25")
    cvar_dmg = register_cvar("m32_dmg","140.0")
    
    //register_clcmd("buyammo1", "clcmd_buyammo")
    //register_clcmd("buyammo2", "clcmd_buyammo")
    gmsgWeaponList = get_user_msgid("WeaponList")
    register_clcmd("give_m32", "give_m322");
    g_MaxPlayers = get_maxplayers()

    g_itemDmg = ctf_item("M32", 250, 100, ADMIN_ALL, "");
}
public give_m322(id) {
    if (is_user_admin(id)) {
        give_m32(id);
...
public pfn_touch(ptr, ptd)
{
    ...
    {   
        ...
        if(equal(classname, "m32_grenade"))
        {
            ...
            while((a = find_ent_in_sphere(a,originF,250.0)) != 0) {
                if( a!=owner&& a!=ptr&&pev(a,pev_takedamage)!=DAMAGE_NO) 
                ExecuteHamB( Ham_TakeDamage, a ,owner ,owner,  get_pcvar_float(cvar_dmg)*1.0, DMG_BULLET )
