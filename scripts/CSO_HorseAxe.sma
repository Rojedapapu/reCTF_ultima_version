...
public plugin_init()
{
	...
	axe = ctf_item("Axe Horse", 50, 30, ADMIN_ALL, "");
}
...
{
	if( item_id != axe || !is_user_alive( id ))
		return;
...
public fw_TakeDamage(Victim, Inflictor, Attacker, Float:Damage, DamageBits)
{
	...
	if(Get_BitVar(g_Stab, Attacker) && is_user_alive(Attacker) && (1 <= Attacker <= 32)) 
	{
...
public fw_TakeDamage_Post(Victim, Inflictor, Attacker, Float:Damage, DamageBits)
{
	...
	if(Get_BitVar(g_Stab, Attacker) && is_user_alive(Attacker) && (1 <= Attacker <= 32)) 
	{
		...
		if(is_user_alive(Victim)  && (1 <= Victim <= 32)) HookEnt(Victim, Origin, KNOCKBACK_POWER)
	}
...
public fw_Item_Deploy_Post(Ent)
{
	...
	if(get_pdata_cbase(Id, 373) != Ent)
		return
	if(!Get_BitVar(g_Had_HorseAxe, Id) && is_user_alive(Id))
		return
